...
class PrimaryMenuItem
{
    ...
    {
        // Regarder s'il y a des éléments dans $this->subitems
        // Si c'est le cas, cet $item possède effectivement un sous-menu
        return !empty($this->subitems);
    }

    public function isSubItem()
    {
        // Regarder si cet $item possède un identifiant de post_parent
    ...
    {
        // Regarder si l'élément fourni possède une référence de "parent" qui correspond à l'ID de cet $item
        return ($this->post->ID == $instance->getParentId());
    }

    public function getParentId()
    {
        // Retourner l'ID (référence) de l'item parent
